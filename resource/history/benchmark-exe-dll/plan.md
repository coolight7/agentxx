...
| FFI | `ffi_api.h` 为唯一稳定 C ABI; `FfiAgentRuntime::buildConfigs` 支持 `plugins/sides/args/dataDir/permissionMode/workDir/modelContextMaxToken`; `ffi_symbols.map` 仅导出 `agentxx_ffi_*` | bench 经 `dlopen(libagentxx.so)+dlsym` 走公开 C API (与外部语言宿主同路径), 不得直链内部 C++ 符号; 上下文经"同模板 mock LLM 驱动 turns"生成以保证内容一致 |
