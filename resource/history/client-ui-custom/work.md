...
- 最后核对: **2026-09-23（阶段 17）** —— 对照 plan.md 逐章核对代码 + 审计遗留项，
  结论见『对照 plan.md 的逐章核对』『状态总览』『遗留任务清单』；
  2026-09-23 完成最后一条可选增强（快捷键列表展示），遗留清单全部闭环
...
| 阶段 17 | 遗留可选增强：快捷键列表展示（设置条目 + 只读列表弹窗 + 键位冲突记录） | ✅ 已完成 |
| 基准 | benchmark「组件密集面板」场景（plan §9 性能预算） | ❌ 未做（评估后不做，见『遗留-基准』） |

小结（阶段 17 结束时）：plan 的 P1 / P2 / P3 全部落地；遗留清单**全部闭环**（真实缺陷、
机制与复用、测试欠账、可选增强、文档残留）；未实施 3 条（解析结果落表 / DSO 测试夹具 /
基准场景）与部分完成 1 条（拖拽宽度→尺寸事件用例）均有成文结论。全量测试
**22696 项断言通过**（无 ASan 报告）。
...
| 可选增强 | 2 | ✅ 2 条完成（overlay 可见性/尺寸 + 快捷键列表展示，阶段 10/17） |
| 文档与注释残留 | 3 | ✅ 3 条完成 |
...
- [x] **快捷键列表展示**（2026-09-23 完成，见『阶段 17』）：设置弹窗新增「快捷键」条目
  （显示插件已注册的条数）+ 只读列表弹窗 `KeybindListOverlay`（键位 + 说明 + 归属插件，
  可滚动）；抢不到键位的尝试以「键位冲突」段展示（申请方 · 占用方），记录来自新增的
  `ClientKeybindConflict`（注册受阻时写入 UI 注册表快照，随占用方注销/禁用或请求方
  禁用/卸载自动清除）
...
**待实施任务 = 0**（全部闭环；本方案的 P1/P2/P3 与全部遗留项均已实施，见『阶段 17』）：

- [x] **快捷键列表展示**（2026-09-23 完成，见『阶段 17』）：设置弹窗「快捷键」条目 +
  只读列表弹窗（键位 + 说明 + 归属插件 + 键位冲突段），不涉及接口表与协议。
...
plan §9.5/§9.6 的最后一处可选增强：让用户**看得见**插件注册的全局快捷键与键位冲突。

### 17.1 数据侧（lib）：键位冲突记录

原来"同键位已被占用"只写 `XX_LOGW`，注册表里查不到 —— 用户只能看到"快捷键不生效"。
新增 `ClientKeybindConflict{keys, plugin, owner}`（`client_plugin_manager.h`），放进
UI 注册表快照 `ClientUiRegistry::keybindConflicts`（与 `keybinds` 同一 COW 快照，
UI 线程一次快照读到两侧信息）：

- 写入：`registerKeybind` 因键位被占用返回 NULL 时记录；**同插件重复注册同一键位不记**
  （那是插件自身行为，不是冲突）；同一 (键位, 请求方) 只保留一条
- 清理：占用方注销键位（键位空出）、占用方被禁用/卸载（`detachDomainRegistrations`）、
  请求方被禁用/卸载（记录作废）三处都清

### 17.2 界面侧（client）

- `KeybindListOverlay`（`components/overlays.{h,cpp}`）：只读列表弹窗
  - 每行 = 键位（强调色加粗，按最长键位对齐，宽字符安全截断/补齐）+ 说明
    （空说明显示占位文案）+ 归属插件（右对齐弱化色）
  - 无注册：显示空状态提示；有冲突：末尾追加「键位冲突 (N)」段
    （`键位` + 申请方 · 占用方）
  - 数据每帧从 `uiRegistrySnapshot()` 读（插件注册/注销/禁用后自动刷新，不触发插件回调）
  - 交互：Up/Down/PageUp/PageDown/Home/End/滚轮滚动，Esc 关闭；面性弹窗外框
    （`tuiSurfacePopup`），条目数不受弹窗高度限制
- 设置弹窗新增「快捷键」条目：值显示已注册条数（每帧刷新），激活打开列表弹窗
  （`SettingsOverlay::onKeybindList` → `TUIClientAgentIO::openKeybindList`，
  与既有 About 同一模态替换语义；模态由 `ModalContainer` 在事件分发期间保活）
- **小终端自适应**：设置条目从 6 条增至 7 条后，弹窗常规版式高度 = 7×2 + 6 + 外框 6 = 26 行，
  在 80×24 这类默认终端上会顶掉底部提示 —— 现按终端高度判断：装不下时把项间距从 1 行
  压到 0（19 行），保证"全部条目 + 底部提示"都可见（再矮的终端仍会被裁切，属既有特性，
  与模型/会话列表弹窗一致）
- i18n：新增 `settings.keybindLabel` / `settings.keybindValue` / `keybind.title` /
  `keybind.hint` / `keybind.empty` / `keybind.noDesc` / `keybind.conflictTitle` /
  `keybind.conflictLine`（zh + en）

### 17.3 测试（+157 项断言，全量 22704 通过）

- `tui_settings`：设置条目条数显示（无插件管理器 / 2 条）+ 点击条目触发打开回调；
  80×24 小终端的条目间距压缩（全部条目与底部提示可见）；列表弹窗空状态与 Esc；
  条目渲染（键位/说明/归属插件/无说明占位）+ 冲突段内容 + 注销后刷新；
  滚动（24 条 / 压低视口：首条不可见→上下键滚动→末条可见、Home/End 跳首尾）
- `tui_surface`：列表弹窗的面性外框区域与"无边框/分割线字符"断言
- `client_plugins`：冲突记录的产生（跨插件占用）、同插件重复注册不记、同一请求方去重、
  占用方注销/被摘除/请求方被摘除三处清理

### 17.4 文档

`plugins.md` §9.6（「界面查看」条目）、`tui.md` §2.6（快捷键列表）、`AGENTS.md`（记忆行）。

### 踩坑（阶段 17）

- **断言屏幕文本不能逐格拼接 `PixelAt(x,y).character`**：宽字符占两格、第二格为空串，
  逐格拼接会把「快捷键」变成「快 捷 键」，子串断言永远失败。用
  `stripAnsi(screen.ToString())`（FTXUI 的 `ToString` 会跳过全宽字符的后一格）；
  需要"哪一行 + 行首列"时用扫行跳过空格子的方式单独定位（点选用行首列坐标）
- 从设置条目里打开另一个模态是**安全**的（`ModalContainer::OnEvent` 局部持有
  `activeModal_` 保活），与 About 的做法一致；但设置条目回调里不要直接 `popModal()`
  后继续访问自身（见 tui.md §2.5 的 `flushActivation` 说明）
- 冲突记录只针对跨插件占用：同插件重复注册同一键位若也记录，列表会出现
  "申请方 X · 占用方 X" 这种自相矛盾的展示
- 设置弹窗的高度是"条目数 × 3 + 外框 6"（条目 2 行 + 间距 1 行）；新增设置项会直接抬高
  最小可用终端高度 —— 加项时留意 80×24 这类默认终端（本次靠"过矮时压缩项间距"兜住，
  更彻底的方案是把条目表接入滚动容器，属独立课题）
