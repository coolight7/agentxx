...
- 滚轮每次 1 行, 事件只累积 `pendingScrollRows_`, **下一帧布局时落实**: 上滚时行偏移
  ...
  先加, 到底进入下一条目首行, 内容底部已进入视口则恢复吸附底部。
...
**测试**: 新增 6 组锚点模型用例 (见下), `agentxx_test` 全量 25698 项断言通过;
`tui_scroll` 760 项历史回归 (抖动/前插/折叠/预算/高视口/流式) 未改动全部通过 ——
说明可达滚动位置集合与旧模型一致 (逐行滚动、滚到顶/底、吸附恢复的语义完全对齐)。
...
| 客户端测试 | — | **25 个模块 2827+ 项** (既有) + 锚点模型 6 组 |
| `agentxx_test` 全量 | — | **25698 项断言全部通过** |
