...
> 通读源码 + 测试 (2026-08) 后记录; 问题按影响程度排序。状态标注: ✅ 已修复 / ⏳ 待修复。

#### 已知问题 (Bug / 风险点)

1. ✅ **WsAgentIOTransport 连接失败后 readLoop 无限重连 (资源泄漏)**
   - 场景: 客户端 `connect()` 握手失败 (鉴权失败/超时) 时, 原 `stopLoops()` 只关闭
     队列/abort wsClient 但**不置 `stopped_`**; readLoop 收到断开后进入重连循环
     (`!stopped_` 恒真), 且 `maxReconnectAttempts=0` 表示无限重试 —— 每 2 秒重连一次
     永不退出。调用方 (`runRemoteCliAsync/runRemoteTuiAsync`) 在 `connect()` 返回 false
     后直接 `co_return`, 不会调用 `close()`; 而 readLoop 经 `shared_from_this` 自持
     transport, 析构也不会发生。
   - 影响: 远程连接失败后遗留一个常驻协程 + 持续的连接尝试, 直至进程退出。
   - 修复 (2026-08): `connect()` 握手失败路径改为调用 `close()` (置 `stopped_` 并
     stopLoops), readLoop 检测到 `stopped_` 后退出且不进入重连循环。

2. ✅ **AgentIOBase::runTransportLoop 循环内使用成员 `transport_` (连接替换场景双接收循环)**
   - `while (transport_->alive()) { co_await transport_->recv(); ... }` —— `recv()` 绑定
     调用时的 transport, 但循环条件与下一次迭代读取的是**成员**。当 AgentServer 同一
     threadId 的新连接替换旧连接 (`ctrl->setTransport(newTransport)` + 关闭旧 transport)
     时, 旧连接的接收协程不会终止: 下一轮循环对新 transport 发起第二个
     `async_receive`, 新旧两个接收循环并发消费同一 transport 的消息 (消息被瓜分,
     行为基本正确但语义混乱), 且旧协程直到新 transport 关闭才退出, `onDisconnect()`
     也被多调用一次 (产生多余 grace 定时器)。
   - 修复 (2026-08): `runTransportLoop` 开头捕获局部 `auto transport = transport_;` 并以
     局部引用循环, 旧协程随旧 transport 关闭自然退出。新增测试
     `test_run_transport_loop_replace_transport` (remote_agent) 覆盖连接替换场景。

3. ⏳ **SubagentSupervisor::runSubagent 取消路径残留 `runningRegistry_` 条目**
   - `runSubagent` 的 catchErrorAsync 未提供 `onRethrow`: 取消/中断类异常 (Cancelled
     /NodeInterrupt) 原样抛出, 而 `runningRegistry_[subagentName] = true` 的擦除只发生
     在成功与 onError 路径 —— 取消后条目残留, 后续 cross-agent 查询误报该 subagent
     "running"。批量委派路径 (runBatch) 同样如此 (取消被转为错误条目但 registry 未清)。
   - 修复建议: 用 RAII guard 擦除, 或在 onRethrow 中擦除。

4. ✅ **InterruptHandleArg::fromJson 对缺省字段抛异常 (防御性)**
   - `result.resultId = data["resultId"].get<std::string>();` —— 当 JSON 缺少
     `resultId` 键时 (外部构造/旧数据), `operator[]` 返回 null, `get<std::string>()`
     抛 type_error。`isAccordingFormat` 仅校验 `name` 字段。当前内部调用方总是通过
     `toJson()` 输出完整字段, 风险低; 但 TUI/StdIO 的 handleInterrupt 解析路径可能
     被远端构造的 JSON 触发。
   - 修复 (2026-08): 改为 `data.value("resultId", std::string{})` 防御缺省键。

5. ⏳ **写/读队列满时消息静默丢弃 (已知问题 #3, 与文档正文引用对应)**
   - WS 传输的 writeQueue (cap=4096) 与 recvQueue (cap=256) 使用 `try_send`, 满时
     丢弃: 高频 Delta (流式 token) 在慢网络下可能丢帧; Channel 传输的 channel
     同样 `try_send`。丢弃后 UI 依赖重连时的 Sync/增量重放校准, 不导致状态错乱,
     但极端流量下会丢消息。可考虑背压 (async_send) 或放大容量。

#### 优化建议 (非 bug)

- **Toolcall 真并行**: `ToolcallWrapNode::baseRun` 目前按顺序 co_await 执行多个
  tool (代码注释 TODO: 真正并行)。可改为 co_spawn + channel 收集, 一轮内的多个
  独立工具 (如并发读多个文件) 可并行执行。
- **`Session::deltaSeq` 注释声称原子, 实现为普通 `uint64_t`** (context.h):
  实际仅 io 线程读写, 无竞争, 但注释误导; 可改 `std::atomic<uint64_t>` 或修正注释。
- **`SessionPersistence::dbs()` 每次写入都 `fs::create_directories`**: 高频路径
  (appendHistory 每消息一次) 存在系统调用开销, 可缓存目录已创建状态。
- **`runConversationTurnAsync` 的 `cfg.max_steps = 1024` 硬编码**: 建议移入
  AgentConfig 可配置。
- **`Session::activity` 字段无读取者**: 仅 LogPrint 中间件写入 (ExecutingTool/Idle),
  UI 状态感知实际经 Wire Delta 完成; 属死字段, 可接入状态栏或移除。
- **StdIO 客户端静态全局计数器** (`g_firstSessionDone`/`g_mcpCount` 等): 多实例
  共享, 首个实例结束后汇总输出失效; 建议移入实例成员。
- **MCP 工具加载为串行循环** (createTools): 多 MCP 服务器可并行 initialize。
