...
- **Conditional dependencies are declared by name only; each consumer locates them on its own machine**: the exported interfaces contain `PkgConfig::uring` (io_uring — used by `cxx_utilxx_base`'s asio async-file-IO probe) and `PkgConfig::hyperscan` plus the bare name `hs_runtime` (HyperScan — used by `cxx_utilxx`'s regex), never a library file path. Whether they are needed is advertised by the util packages' configs (`cxx_utilxx_base_LINUX_IO_URING_SUPPORTED` / `cxx_utilxx_ENABLE_HYPERSCAN`). Static archives do not carry dependency binaries, so whoever links must resolve them, and each build directory writes the `pkg_check_modules` calls directly in its own `CMakeLists.txt` (dependency libraries first, then `find_package` for the util libraries / `agentxx_static`): lib/client/test/benchmark/plugins key off the top-level `AGENTXX_LINUX_IO_URING_SUPPORTED` / `AGENTXX_ENABLE_HYPERSCAN` switches (one lookup in the plugins directory covers every plugin target, so plugins need no per-plugin configuration), while the two util libraries key off the flag exported by `cxx_utilxx_base`'s package config. A missing lookup fails loudly at configure time, or at `dlopen` time with `undefined symbol: io_uring_queue_init`.
