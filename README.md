...
|---|---|---|---|---|
| **Win/TUI** | 4.2M | 13.2M | 21.5M | 任务管理器查看内存占用 |
| **Linux/TUI** | 3.6M | 9.2M | 15.1M | top命令查看RES-SHR, agentxx_cli 仅依赖系统库，不需要其他动态库，因此仅计算独占内存大小 |
...
- ✅TUI: `agentxx_cli tui`
    ...
    - 支持 白昼、夜间 主题
    - 支持消息列表/会话列表局部分页加载
    - 支持显示思考内容、流式显示、Markdown渲染、状态图渲染
    - 支持选择流式渲染思考内容方式（单行裁剪显示末尾内容/自动展开显示完整内容，完成时自动折叠）
    - 支持显示 LLM Api token速度、耗时、轮次完成时间点
    - 可选动画等级
- ✅Cli: `agentxx_cli cli`
    - 多用于辅助调试测试，非着重开发方向, 建议使用 TUI
