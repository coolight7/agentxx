...
static bool tryInvokePluginCommand(
    ...
    const auto cmdName  = input.substr(
        1,
...
static asio::awaitable<void> runLocalCliUnifiedAsync(
    std::shared_ptr<agent::CodeAgent> agent,
...
static asio::awaitable<void> runLocalTuiUnifiedAsync(
    ...
    auto pluginMgr = setupClientPlugins<TUIClientAgentIO, TuiPluginAdapter>(
        clientEx, tui, threadId, plugins
    );
    tui->start();
    co_await pluginMgr->loadConfiguredClientPlugins(plugins);
...
static asio::awaitable<void>
    runRemoteCliAsync(std::string url, std::string token, std::string model, ClientPluginConfigs plugins) {
...
void runRemoteCli(std::string_view url, std::string_view token, std::string_view model, ClientPluginConfigs plugins) {
    asio::io_context ctx;
    asio::co_spawn(
        ctx,
        runRemoteCliAsync(
