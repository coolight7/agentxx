...
struct PluginCtx {
    ...
    volatile int usage_cancel_flag = 0;
    /// 采集/发布开关 (client /sysinfo 经 usage_enabled 事件同步)
    std::atomic<bool> usageEnabled{true};
    /// 宿主周期定时器句柄 (v7 add_timer; destroy 时 cancel_timer)
    void* timer = nullptr;
...
static void pluginLog(const PluginCtx* ctx, int level, const std::string& msg) {
    if (ctx) {
        agentxx_system_monitor_plugin::pluginLog(ctx->host, ctx->iface.log, level, msg);
    }
}
...
static void registerTool(
    ...
    if (agentxx_register_sync_tool(ctx.host, &spec, shim) != 0) {
        pluginLog(&ctx, 3, fmt::format("agentxx_system_monitor: register tool {} failed", name));
    }
}
...
static void ensureToolPromptInHost(PluginCtx& ctx) {
    ...
    if (ctx.iface.prompt->set_prompt(ctx.host, agentxx_plugin_sv(payload.data(), payload.size()))
        != 0) {
        pluginLog(&ctx, 3, "agentxx_system_monitor: set_prompt failed");
    }
}
...
static char* getSystemCoreInfoExecute(
    ...
    auto* ctx = static_cast<PluginCtx*>(user_data); ///< 多实例契约: 结果串走本实例宿主堆
    (void)args_json;
    (void)thread_id;
    (void)tool_call_id;
    (void)cancel_flag;
    try {
        ...
        return pluginStrdup(ctx ? ctx->host : nullptr, ss.str().c_str());
...
static void* usageCollectWork(void* ud, volatile int* cancel_flag, char** error_out) {
    ...
    try {
        auto usage = querySync();
        auto json  = usageToJson(*ctx, usage);
        // 结果经宿主分配 (跨 CRT 堆边界)
        char* p = static_cast<char*>(host->vtable->alloc(json.size() + 1));
        if (p) {
        ...
        if (error_out) {
            *error_out = host->vtable->strdup(e.what());
        }
        return nullptr;
    } catch (...) {
        if (error_out) {
            *error_out = host->vtable->strdup("unknown error");
        }
        return nullptr;
    }
}
...
static void refreshUsageDisplay(ClientCtx& ctx) {
    ...
    if (ctx.section && !ctx.last_usage_json.empty() && ctx.ui && ctx.ui->update_info_section) {
        std::string json;
...
static char* sysinfo_cmd_execute(void* ud, AgentxxPluginStringView args_json, char** error_out) {
    auto* ctx = static_cast<ClientCtx*>(ud); ///< 多实例契约: 开关挂本实例
    (void)args_json;
    (void)error_out;
    if (!ctx || !ctx->host) {
        return nullptr;
