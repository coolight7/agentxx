...
void AGENTXX_PLUGIN_CALL onScriptLoadDone(
    ...
    if (status == AGENTXX_PLUGIN_OPERATOR_OK) {
        if (ctx) {
            ...
            if (!text.empty()) {
                shellLog(ctx, 2, fmt::format("example_js: loaded tools {}", text));
            }
        }
    ...
    if (ctx) {
        shellLog(ctx, 4, fmt::format("example_js: interpreter load failed: {}", msg));
    }
...
void AGENTXX_PLUGIN_CALL onScriptUnloadDone(
    ...
    shellLog(
        ctx,
        3,
        fmt::format("example_js: interpreter unload failed: {}", text.empty() ? "?" : text)
    );
}
...
void dispatchScriptUnload(ShellCtx& ctx) {
    ...
    if (err.data) {
        if (!h) {
            shellLog(&ctx, 3, fmt::format("example_js: unload dispatch failed: {}", err.data));
        }
    ...
    return agentxx::plugin::guardCall(
        ...
        [&]() -> const AgentxxPluginInfo* {
            static const AgentxxPluginInfo info{
                ...
                0,
                agentxx::plugin::PluginStringView::fromCstr("example_js"),
...
/// - 加载经 "interpreter.js" 能力异步执行, 脚本注册的工具/订阅属于本实例;
...
static void* jsShellAgentStart(
    ...
    if (!notify || !notify->done) {
        return setErr("example_js start: notify required");
    }
    if (!ctx.iface.capabilities || !ctx.iface.capabilities->has_capability
        || !ctx.iface.capabilities->invoke_capability_async) {
        return setErr("example_js start: host lacks capabilities interface");
    }
    auto capSv = agentxx::plugin::PluginStringView::fromCstr("interpreter.js");
    if (!ctx.iface.capabilities->has_capability(ctx.host, &capSv)) {
        return setErr("example_js start: interpreter.js capability not available");
    }
    if (!fileExists(ctx.scriptPath)) {
        return setErr("example_js start: plugin.js not found next to the plugin library");
    }
    if (ctx.scriptLoaded) {
    ...
    shellLog(&ctx, 2, fmt::format("example_js: loading script {}", ctx.scriptPath));
