...
static ToolPrompt defaultToolPrompt(std::string_view name) {
    ...
    if (name == "agentxx_codegraph_search") {
        p.depict = "Search for code symbols (functions, classes, variables, etc.) by name using the "
                   "codegraph index.\n"
        ...
        p.depict   = "Get rich context for a code symbol: its definition, callers, callees, and methods "
                     "(for classes).\n"
        ...
        p.depict   = "Find all functions that call a given symbol (reverse call-graph traversal).\n"
        ...
        p.depict   = "Find all functions that a given symbol calls (forward call-graph traversal).\n"
        ...
        p.depict = "Analyze the impact of modifying a symbol. Finds all downstream symbols that may be\n"
        ...
        p.depict = "Find the call-chain path between two symbols in the call graph.\n"
...
static std::string impactToText(
    std::string_view       title,
    const codegraph::Json& impact,
    CodeGraphManager*      m
) {
    std::string out;
...
struct SchemaBuilder {
    ...
    void str(std::string_view key, std::string_view desc) {
        properties[std::string(key)] = codegraph::Json{{"type", "string"}, {"description", std::string(desc)}};
    }
    void num(std::string_view key, std::string_view desc) {
        properties[std::string(key)] = codegraph::Json{{"type", "number"}, {"description", std::string(desc)}};
    }
    std::string dump(std::initializer_list<std::string_view> req = {}) const {
        ...
        if (req.size() > 0) {
            ...
            for (auto r : req) arr.push_back(codegraph::Json(std::string(r)));
            s["required"] = arr;
        }
        return s.dump();
    }
...
static void registerAllTools(PluginCtx& ctx) {
    ...
    // agentxx_codegraph_search
    {
        ...
        std::string depict = p.depict.empty() ? defaultToolPrompt("agentxx_codegraph_search").depict : p.depict;
        SchemaBuilder b;
        b.str("query", p.args.contains("query") ? p.args["query"] : "Symbol name to search for. Supports partial matching.");
        b.num("limit", p.args.contains("limit") ? p.args["limit"] : "Maximum number of results to return. Default: 20.");
    ...
    // agentxx_codegraph_context
    {
        ...
        std::string depict = p.depict.empty() ? defaultToolPrompt("agentxx_codegraph_context").depict : p.depict;
        SchemaBuilder b;
        b.str("symbol", p.args.contains("symbol") ? p.args["symbol"] : "Target symbol name.");
        b.num("limit", p.args.contains("limit") ? p.args["limit"] : "Maximum results per category. Default: 10.");
        b.num("max_depth", p.args.contains("max_depth") ? p.args["max_depth"] : "Maximum traversal depth. Default: 3.");
        agentxx::kit::blocking_tool(
            ctx,
            "agentxx_codegraph_context",
            depict,
            ...
            [](PluginCtx& c, std::string_view args_json) -> std::string {
                ...
                if (symbol.empty()) return "error: Arg `symbol` is empty";
                int64_t limit64 = 10, depth64 = 3;
                jsonGetInt(a.doc().at_pointer("/limit"), limit64);
                jsonGetInt(a.doc().at_pointer("/max_depth"), depth64);
                auto r = c.mgr->getSymbolContext(symbol, static_cast<int>(limit64), static_cast<int>(depth64));
                if (!r.success) return fmt::format("error: {}", r.error);
    ...
    // agentxx_codegraph_callers
    {
        ...
        std::string depict = p.depict.empty() ? defaultToolPrompt("agentxx_codegraph_callers").depict : p.depict;
        SchemaBuilder b;
        b.str("symbol", p.args.contains("symbol") ? p.args["symbol"] : "Target symbol name.");
        b.num("max_depth", p.args.contains("max_depth") ? p.args["max_depth"] : "Maximum traversal depth. Default: 3.");
        agentxx::kit::blocking_tool(
    ...
    // agentxx_codegraph_callees
    {
        ...
        std::string depict = p.depict.empty() ? defaultToolPrompt("agentxx_codegraph_callees").depict : p.depict;
        SchemaBuilder b;
        b.str("symbol", p.args.contains("symbol") ? p.args["symbol"] : "Target symbol name.");
        b.num("max_depth", p.args.contains("max_depth") ? p.args["max_depth"] : "Maximum traversal depth. Default: 3.");
        agentxx::kit::blocking_tool(
    ...
    // agentxx_codegraph_impact
    {
        ...
        std::string depict = p.depict.empty() ? defaultToolPrompt("agentxx_codegraph_impact").depict : p.depict;
        SchemaBuilder b;
        b.str("symbol", p.args.contains("symbol") ? p.args["symbol"] : "Target symbol name.");
        b.num("max_depth", p.args.contains("max_depth") ? p.args["max_depth"] : "Maximum traversal depth. Default: 5.");
        agentxx::kit::blocking_tool(
            ctx,
            "agentxx_codegraph_impact",
            depict,
    ...
    // agentxx_codegraph_path
    {
        ...
        std::string depict = p.depict.empty() ? defaultToolPrompt("agentxx_codegraph_path").depict : p.depict;
        SchemaBuilder b;
        b.str("from", p.args.contains("from") ? p.args["from"] : "Starting symbol name.");
        b.str("to", p.args.contains("to") ? p.args["to"] : "Target symbol name.");
        b.num("max_depth", p.args.contains("max_depth") ? p.args["max_depth"] : "Maximum search depth. Default: 10.");
        agentxx::kit::blocking_tool(
            ctx,
            "agentxx_codegraph_path",
            depict,
            ...
            [](PluginCtx& c, std::string_view args_json) -> std::string {
                ...
                if (from.empty() || to.empty()) return "error: Args `from` and `to` are required";
                int64_t depth64 = 10;
                jsonGetInt(a.doc().at_pointer("/max_depth"), depth64);
...
static void snapshotQueryDone(void* ud, void* result, char* error) {
    ...
    if (ctx && ctx->host && ctx->host->vtable && ctx->host->vtable->free) {
        if (files) ctx->host->vtable->free(files);
        if (error) ctx->host->vtable->free(error);
    }
}
...
static void* snapshotQueryWork(void* ud, volatile int*, char**) {
    ...
    return agentxx::plugin_guard::guardCall(
        ...
        nullptr,
        [&]() -> void* {
        auto files = ctx ? static_cast<int64_t*>(ctx->host->vtable->alloc(sizeof(int64_t))) : nullptr;
        if (files) {
            *files = -1;
            if (ctx && ctx->mgr) {
                auto st = ctx->mgr->getStatus();
                if (st.success) *files = st.total_files;
            }
        }
        return files;
    });
...
static void on_client_attached(AgentxxPluginStringView, void* ud) {
    ...
    agentxx::plugin_guard::guardCallVoid(
        ...
        [&] {
        if (!ctx || !ctx->host || !ctx->iface.scheduler || !ctx->iface.scheduler->offload) return;
...
extern "C" AGENTXX_PLUGIN_EXPORT int
    ...
    return agentxx::plugin_guard::guardCall(
        ...
        if (cfg.dataDir.empty()) {
            pluginLog(ctx->host, ctx->iface.log, 3, "agentxx_codegraph: dataDir not set, skip tools");
        ...
        ctx->mgr->setLogSink([ctxPtr = ctx.get()](int level, const std::string& msg) {
            pluginLog(ctxPtr ? ctxPtr->host : nullptr, ctxPtr ? ctxPtr->iface.log : nullptr, level, msg);
        });

        PluginCtx* ctxRaw = ctx.get();
        ctx->mgr->setProgressCallback([ctxRaw](int processed, int total, std::string_view currentFile) {
            try {
                if (!ctxRaw || !ctxRaw->host || !ctxRaw->iface.events || !ctxRaw->iface.events->publish) return;
                codegraph::Json j   = codegraph::Json::object();
        ...
        ctx->warmup = std::thread([ctxPtr = ctx.get()]() {
            try {
                ...
                if (ctxPtr->stop.load(std::memory_order_acquire)) return;
                auto t0 = std::chrono::steady_clock::now();
                bool ok = ctxPtr->mgr->updateIndex();
                auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t0).count();
                pluginLog(ctxPtr->host, ctxPtr->iface.log, ok ? 2 : 3,
                          fmt::format("[codegraph] background warmup index {} ({}ms)", ok ? "done" : "failed", ms));
            } catch (...) {}
        });

        if (ctx->iface.events && ctx->iface.events->subscribe) {
            ctx->iface.events->subscribe(host, AGENTXX_SV("agentxx_host.client_attached"), on_client_attached, ctx.get());
        }
