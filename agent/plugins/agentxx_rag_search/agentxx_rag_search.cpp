...
extern "C" AGENTXX_PLUGIN_EXPORT int agentxx_plugin_create(const AgentxxHost* host, void** plugin_ctx) {
    ...
    return agentxx::plugin_guard::guardCall(
        ...
        pluginLog(ctx.get(),
                  isAddSuccess ? 2 : 3,
                  fmt::format("RAG: loading {} documents to vector index {}",
                              docxSize,
        ...
        std::string schema = neograph::json{
            ...
            {"properties", {
                {"query", {
                    {"type", "string"},
                    {"description", argDesc(p, "query", "Search query text to find relevant documents.")}
                }},
                {"top_k", {
                    {"type", "integer"},
                    {"default", 3},
                    {"description", argDesc(p, "top_k", "Number of top relevant results to return (default 3, min 1, max 50).")}
                }}
            }},
