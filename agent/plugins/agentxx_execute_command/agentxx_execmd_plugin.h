...
struct PluginCtx {
    ...
    /// 交错执行, 零额外线程; 无 bp::v2 的 popen 回退路径不使用)
...
inline std::string readWorkDir(const PluginCtx* ctx, AgentxxPluginStringView tid = {}) {
    if (!ctx || !ctx->iface.config) {
        return {};
    }
    if (ctx->iface.config->get_session_work_dir && tid.data && tid.size > 0) {
        char* dir = ctx->iface.config->get_session_work_dir(ctx->host, tid);
        if (dir) {
            std::string out{dir};
    ...
    if (!ctx->iface.config->get_work_dir) {
        return {};
    }
...
inline bool isSessionCancelled(const PluginCtx* ctx, AgentxxPluginStringView thread_id) {
    if (!ctx || !ctx->iface.cancel || !ctx->iface.cancel->is_cancelled
        || agentxx_plugin_sv_empty(thread_id)) {
        return false;
    }
    return ctx->iface.cancel->is_cancelled(ctx->host, thread_id) != 0;
}
