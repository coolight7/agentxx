...
static void checkResult(const std::string& actual, const std::string& expected, const std::string& testName) {
    if (actual == expected) {
        g_math_passed++;
...
static void checkContains(const std::string& actual, const std::string& substr, const std::string& testName) {
    if (actual.find(substr) != std::string::npos) {
        g_math_passed++;
        TEST_PASS << testName << " contains '" << substr << "' -> " << actual << std::endl;
    } else {
        g_math_failed++;
        std::cout << "[FAIL] " << testName << " expected to contain '" << substr << "', got: '" << actual << "'" << std::endl;
    }
}
...
static asio::awaitable<void> test_basic_arithmetic(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    ...
    auto r1 = co_await tool.execute_async({{"expression", "2 + 3 * 4"}});
    checkResult(r1, "14", "2 + 3 * 4");

    auto r2 = co_await tool.execute_async({{"expression", "(2 + 3) * 4"}});
    checkResult(r2, "20", "(2 + 3) * 4");

    auto r3 = co_await tool.execute_async({{"expression", "10 - 4 - 2"}});
    checkResult(r3, "4", "10 - 4 - 2");

    auto r4 = co_await tool.execute_async({{"expression", "15 / 4"}});
    checkResult(r4, "3.75", "15 / 4");

    auto r5 = co_await tool.execute_async({{"expression", "15 // 4"}});
    checkResult(r5, "3", "15 // 4 (floor div)");

    auto r6 = co_await tool.execute_async({{"expression", "15 % 4"}});
    checkResult(r6, "3", "15 % 4 (modulo)");

    auto r7 = co_await tool.execute_async({{"expression", "-5 + 8"}});
    checkResult(r7, "3", "-5 + 8");

    auto r8 = co_await tool.execute_async({{"expression", "-(-5)"}});
    checkResult(r8, "5", "-(-5)");
...
// 2. 乘方与阶乘
static asio::awaitable<void> test_power_and_factorial(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    auto tool = agentxx::tools::MathCalculateTool{ctx};

    auto r1 = co_await tool.execute_async({{"expression", "2^10"}});
    checkResult(r1, "1024", "2^10");

    auto r2 = co_await tool.execute_async({{"expression", "2**10"}});
    checkResult(r2, "1024", "2**10");

    // 右结合 2^3^2 = 2^(3^2) = 2^9 = 512
    auto r3 = co_await tool.execute_async({{"expression", "2^3^2"}});
    checkResult(r3, "512", "2^3^2 right-associative");

    auto r4 = co_await tool.execute_async({{"expression", "5!"}});
    checkResult(r4, "120", "5!");

    auto r5 = co_await tool.execute_async({{"expression", "0!"}});
    checkResult(r5, "1", "0!");

    auto r6 = co_await tool.execute_async({{"expression", "(3 + 2)!"}});
    checkResult(r6, "120", "(3 + 2)!");

    auto r7 = co_await tool.execute_async({{"expression", "2^3!"}});
...
// 3. 隐式乘法
static asio::awaitable<void> test_implicit_multiplication(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    auto tool = agentxx::tools::MathCalculateTool{ctx};

    auto r1 = co_await tool.execute_async({{"expression", "2(3 + 4)"}});
    checkResult(r1, "14", "2(3 + 4)");

    auto r2 = co_await tool.execute_async({{"expression", "(1 + 2)(3 + 4)"}});
    checkResult(r2, "21", "(1 + 2)(3 + 4)");

    auto r3 = co_await tool.execute_async({{"expression", "3sqrt(16)"}});
    checkResult(r3, "12", "3sqrt(16)");

    auto r4 = co_await tool.execute_async({{"expression", "2[3 + 4]"}});
...
// 4. 常量与基础函数
static asio::awaitable<void> test_constants_and_functions(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    auto tool = agentxx::tools::MathCalculateTool{ctx};

    auto r1 = co_await tool.execute_async({{"expression", "sqrt(144)"}});
    checkResult(r1, "12", "sqrt(144)");

    auto r2 = co_await tool.execute_async({{"expression", "cbrt(27)"}});
    checkResult(r2, "3", "cbrt(27)");

    auto r3 = co_await tool.execute_async({{"expression", "abs(-42)"}});
    checkResult(r3, "42", "abs(-42)");

    auto r4 = co_await tool.execute_async({{"expression", "floor(3.9)"}});
    checkResult(r4, "3", "floor(3.9)");

    auto r5 = co_await tool.execute_async({{"expression", "ceil(3.1)"}});
    checkResult(r5, "4", "ceil(3.1)");

    auto r6 = co_await tool.execute_async({{"expression", "round(3.567, 2)"}});
    checkResult(r6, "3.57", "round(3.567, 2)");

    auto r7 = co_await tool.execute_async({{"expression", "log10(1000)"}});
    checkResult(r7, "3", "log10(1000)");

    auto r8 = co_await tool.execute_async({{"expression", "log2(256)"}});
    checkResult(r8, "8", "log2(256)");

    auto r9 = co_await tool.execute_async({{"expression", "exp(0)"}});
    checkResult(r9, "1", "exp(0)");

    auto r10 = co_await tool.execute_async({{"expression", "ln(e)"}});
    checkResult(r10, "1", "ln(e)");

    auto r11 = co_await tool.execute_async({{"expression", "min(5, 2, 8, 1, 9)"}});
    checkResult(r11, "1", "min(...)");

    auto r12 = co_await tool.execute_async({{"expression", "max(5, 2, 8, 1, 9)"}});
    checkResult(r12, "9", "max(...)");

    auto r13 = co_await tool.execute_async({{"expression", "sum(1, 2, 3, 4, 5)"}});
    checkResult(r13, "15", "sum(...)");

    auto r14 = co_await tool.execute_async({{"expression", "avg(10, 20, 30)"}});
    checkResult(r14, "20", "avg(...)");

    auto r15 = co_await tool.execute_async({{"expression", "gcd(48, 18)"}});
    checkResult(r15, "6", "gcd(48, 18)");

    auto r16 = co_await tool.execute_async({{"expression", "lcm(12, 18)"}});
    checkResult(r16, "36", "lcm(12, 18)");

    auto r17 = co_await tool.execute_async({{"expression", "comb(5, 2)"}});
    checkResult(r17, "10", "comb(5, 2)");

    auto r18 = co_await tool.execute_async({{"expression", "perm(5, 2)"}});
    checkResult(r18, "20", "perm(5, 2)");

    auto r19 = co_await tool.execute_async({{"expression", "clamp(15, 0, 10)"}});
    checkResult(r19, "10", "clamp(15, 0, 10)");

    auto r20 = co_await tool.execute_async({{"expression", "hypot(3, 4)"}});
...
static asio::awaitable<void> test_trigonometric(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    ...
    // 默认弧度
    auto r1 = co_await tool.execute_async({{"expression", "sin(pi / 2)"}});
    checkResult(r1, "1", "sin(pi / 2)");

    auto r2 = co_await tool.execute_async({{"expression", "cos(0)"}});
    checkResult(r2, "1", "cos(0)");

    // 显式角度函数 sind, cosd
    auto r3 = co_await tool.execute_async({{"expression", "sind(90)"}});
    checkResult(r3, "1", "sind(90)");

    auto r4 = co_await tool.execute_async({{"expression", "cosd(180)"}});
    checkResult(r4, "-1", "cosd(180)");

    auto r5 = co_await tool.execute_async({{"expression", "tand(45)"}});
    checkResult(r5, "1", "tand(45)");

    // angle_unit = "deg"
    auto r6 = co_await tool.execute_async({{"expression", "sin(30)"}, {"angle_unit", "deg"}, {"precision", 2}});
    checkResult(r6, "0.50", "sin(30) with angle_unit=deg");

    auto r7 = co_await tool.execute_async({{"expression", "asin(0.5)"}, {"angle_unit", "deg"}});
    checkResult(r7, "30", "asin(0.5) with angle_unit=deg");

    // rad / deg 转换函数
    auto r8 = co_await tool.execute_async({{"expression", "deg(pi)"}});
    checkResult(r8, "180", "deg(pi)");

    auto r9 = co_await tool.execute_async({{"expression", "rad(180) / pi"}});
...
static asio::awaitable<void> test_bitwise_and_bases(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    ...
    auto r1 = co_await tool.execute_async({{"expression", "0xFF"}});
    checkResult(r1, "255", "0xFF");

    auto r2 = co_await tool.execute_async({{"expression", "0b101010"}});
    checkResult(r2, "42", "0b101010");

    auto r3 = co_await tool.execute_async({{"expression", "0o77"}});
    checkResult(r3, "63", "0o77");

    auto r4 = co_await tool.execute_async({{"expression", "12 & 10"}});
    checkResult(r4, "8", "12 & 10");

    auto r5 = co_await tool.execute_async({{"expression", "12 | 10"}});
    checkResult(r5, "14", "12 | 10");

    auto r6 = co_await tool.execute_async({{"expression", "xor(12, 10)"}});
    checkResult(r6, "6", "xor(12, 10)");

    auto r7 = co_await tool.execute_async({{"expression", "1 << 8"}});
    checkResult(r7, "256", "1 << 8");

    auto r8 = co_await tool.execute_async({{"expression", "1024 >> 3"}});
    checkResult(r8, "128", "1024 >> 3");

    auto r9 = co_await tool.execute_async({{"expression", "~0"}});
    checkResult(r9, "-1", "~0");
...
static asio::awaitable<void> test_logic_and_ternary(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    ...
    auto r1 = co_await tool.execute_async({{"expression", "5 > 3"}});
    checkResult(r1, "1", "5 > 3");

    auto r2 = co_await tool.execute_async({{"expression", "5 == 5"}});
    checkResult(r2, "1", "5 == 5");

    auto r3 = co_await tool.execute_async({{"expression", "5 != 3"}});
    checkResult(r3, "1", "5 != 3");

    auto r4 = co_await tool.execute_async({{"expression", "1 && 0"}});
    checkResult(r4, "0", "1 && 0");

    auto r5 = co_await tool.execute_async({{"expression", "1 || 0"}});
    checkResult(r5, "1", "1 || 0");

    auto r6 = co_await tool.execute_async({{"expression", "!0"}});
    checkResult(r6, "1", "!0");

    auto r7 = co_await tool.execute_async({{"expression", "10 > 5 ? 100 : 200"}});
    checkResult(r7, "100", "10 > 5 ? 100 : 200");

    auto r8 = co_await tool.execute_async({{"expression", "3 > 5 ? 100 : 200"}});
...
// 8. 精度格式化与特殊值
static asio::awaitable<void> test_precision_and_special_values(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    auto tool = agentxx::tools::MathCalculateTool{ctx};

    auto r1 = co_await tool.execute_async({{"expression", "1 / 3"}, {"precision", 4}});
    checkResult(r1, "0.3333", "1 / 3 with precision=4");

    auto r2 = co_await tool.execute_async({{"expression", "pi"}, {"precision", 6}});
    checkResult(r2, "3.141593", "pi with precision=6");

    auto r3 = co_await tool.execute_async({{"expression", "inf"}});
    checkResult(r3, "Infinity", "inf");

    auto r4 = co_await tool.execute_async({{"expression", "-inf"}});
    checkResult(r4, "-Infinity", "-inf");

    auto r5 = co_await tool.execute_async({{"expression", "nan"}});
...
static asio::awaitable<void> test_error_handling(std::weak_ptr<agentxx::agent::AgentContext> ctx) {
    ...
    // 表达式为空
    auto r1 = co_await tool.execute_async({{"expression", ""}});
    checkContains(r1, "empty", "empty expression");

    // 除以零
    auto r2 = co_await tool.execute_async({{"expression", "10 / 0"}});
    checkContains(r2, "Division by zero", "division by zero");

    // 模零
    auto r3 = co_await tool.execute_async({{"expression", "10 % 0"}});
    ...
    // 未知函数
    auto r5 = co_await tool.execute_async({{"expression", "unknown_fn(123)"}});
    checkContains(r5, "Unknown function", "unknown function");

    // 负数开方 (定义域错误)
    auto r6 = co_await tool.execute_async({{"expression", "sqrt(-4)"}});
    checkContains(r6, "Domain error", "sqrt of negative");

    // 负数阶乘
    auto r7 = co_await tool.execute_async({{"expression", "(-3)!"}});
    checkContains(r7, "Factorial", "negative factorial");

    // 非法字符
    auto r8 = co_await tool.execute_async({{"expression", "2 @ 3"}});
...
static asio::awaitable<void> test_plugin_dynamic_load(std::weak_ptr<agentxx::agent::AgentContext> agentContext) {
    ...
    if (inst) {
        ...
        TEST_INFO << "agentxx_math plugin dynamic library not found at: " << path << " (skipping dynamic load test)" << std::endl;
    }
}
...
asio::awaitable<TestResult>
    ...
    std::cout << "--- math_tools done: passed=" << g_math_passed
              << " failed=" << g_math_failed << " ---" << std::endl;
