...
asio::awaitable<void> test_fixed_length_basic() {
    ...
    if (result.size() == 3 && result[0] == "Hello" && result[1] == " Worl" && result[2] == "d") {
        g_rag_passed++;
        TEST_PASS << "splitStringByFixedLength basic" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength basic, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_empty() {
    ...
    if (result.empty()) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength empty should return empty" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_utf8() {
    ...
    if (result.size() == 3 && result[0] == "你好世界" && result[1] == "Hell" && result[2] == "o") {
        g_rag_passed++;
        TEST_PASS << "splitStringByFixedLength utf8" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength utf8, got " << result.size() << " chunks:";
        for (auto& s : result) {
            std::cout << " [" << s << "]";
        }
        std::cout << std::endl;
...
asio::awaitable<void> test_fixed_length_smaller_than_one_char() {
    ...
    if (result.size() == 2 && result[0] == "你" && result[1] == "好") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength single char boundary, got " << result.size()
                  << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_shorter_than_block() {
    ...
    if (result.size() == 1 && result[0] == "Hi") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength shorter than block, got " << result.size()
                  << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_exact_boundary() {
    ...
    if (result.size() == 1 && result[0] == "ABCDE") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength exact boundary, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_mixed_ascii_utf8() {
    ...
    if (result.size() == 3 && result[0] == "AB你" && result[1] == "好CD" && result[2] == "世界") {
        g_rag_passed++;
        TEST_PASS << "splitStringByFixedLength mixed ascii utf8" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength mixed ascii utf8, got " << result.size()
                  << " chunks:";
        for (auto& s : result) {
            std::cout << " [" << s << "]";
        }
...
asio::awaitable<void> test_fixed_length_single_char() {
    ...
    if (result.size() == 1 && result[0] == "X") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitStringByFixedLength single char, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_basic() {
    ...
    if (result.size() == 3 && result[0] == "a" && result[1] == "b" && result[2] == "c") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter basic, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_no_match() {
    ...
    if (result.size() == 1 && result[0] == "no delimiter here") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter no match, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_empty_delim() {
    ...
    if (result.size() == 1 && result[0] == "hello world") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter empty delimiter, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_chinese_punctuation() {
    ...
    if (result.size() == 3 && result[0] == "第一句" && result[1] == "第二句"
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter Chinese period, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_comma() {
    ...
    if (result.size() == 3 && result[0] == "apple" && result[1] == "banana"
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter comma, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_at_start() {
    ...
    if (result.size() == 1 && result[0] == "start") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter at start, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_at_end() {
    ...
    if (result.size() == 1 && result[0] == "end") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter at end, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_consecutive() {
    ...
    if (result.size() == 2 && result[0] == "a" && result[1] == "b") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter consecutive, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_only_delim() {
    ...
    if (result.empty()) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter only delimiter, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_multibyte_utf8() {
    auto result = VectorStore::splitByDelimiter("第一部分¶¶第二部分¶¶第三部分", "¶¶");
    if (result.size() == 3 && result[0] == "第一部分" && result[1] == "第二部分"
        && result[2] == "第三部分") {
        g_rag_passed++;
        TEST_PASS << "splitByDelimiter multibyte utf8" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter multibyte utf8, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiter_multiple_chinese() {
    ...
    if (result.size() == 3 && result[0] == "你好" && result[1] == "世界" && result[2] == "测试") {
        g_rag_passed++;
        TEST_PASS << "splitByDelimiter multiple Chinese excl" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiter multiple Chinese excl, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_headings() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure headings, got " << result.size() << " blocks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_code_block() {
    ...
    if (hasCodeBlock && result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure code block, got " << result.size() << " blocks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_list() {
    ...
    for (auto& block : result) {
        if (block.find("item1") != std::string::npos && block.find("item2") != std::string::npos) {
            hasListBlock = true;
        }
    }
    if (hasListBlock) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure list, got " << result.size() << " blocks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_numbered_list() {
    ...
    bool        hasListBlock = false;
    for (auto& block : result) {
        if (block.find("first") != std::string::npos && block.find("second") != std::string::npos) {
            hasListBlock = true;
        }
    }
    if (hasListBlock) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure numbered list, got " << result.size() << " blocks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_mixed_headings() {
    ...
    if (result.size() >= 3) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure mixed headings, got " << result.size() << " blocks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_single_paragraph() {
    ...
    if (result.size() == 1 && result[0] == text) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure single paragraph, got " << result.size() << " blocks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_many_paragraphs() {
    ...
    if (result.size() == 5) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure many paragraphs, got " << result.size() << " blocks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_blockquote() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure blockquote, got " << result.size() << " blocks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_table() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure table, got " << result.size() << " blocks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_structure_heading_merge() {
    ...
    if (headingMerged) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByStructure heading merge, got " << result.size() << " blocks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_basic() {
    ...
    auto result = VectorStore::splitByDelimiters("para1\n\npara2\n\npara3", 256, delims);
    if (result.size() == 3) {
        g_rag_passed++;
        TEST_PASS << "splitByDelimiters basic" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters basic, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_fallback_to_fixed() {
    ...
    if (result.size() >= 5) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters fallback to fixed, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_within_limit() {
    ...
    if (result.size() == 1 && result[0] == "short") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters within limit, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_empty_text() {
    ...
    if (result.empty()) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters empty text, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_empty_delims() {
    ...
    if (result.size() == 1 && result[0] == "hello world") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters empty delimiters, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_priority() {
    ...
    if (result.size() == 3 && result[0] == "a" && result[1] == "b\nc" && result[2] == "d") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters priority, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_delimiters_chinese() {
    ...
    if (result.size() == 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByDelimiters Chinese, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_fixed_length() {
    ...
    auto result          = VectorStore::splitTextToChunks("Hello World Hello World", config);
    if (result.size() == 3) {
        g_rag_passed++;
        TEST_PASS << "splitTextToChunks FixedLength" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks FixedLength, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_character() {
    ...
    auto result          = VectorStore::splitTextToChunks("para1\n\npara2\n\npara3", config);
    if (result.size() == 3) {
        g_rag_passed++;
        TEST_PASS << "splitTextToChunks Character" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks Character, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_structural() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks Structural, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_structural_then_char() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks StructuralThenChar, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_structural_then_char_then_fixed() {
    VectorStore::SplitConfig config;
    config.mode          = RAGSearchTool::VectorStore::SplitMode::StructuralThenCharThenFixed;
    config.maxUtf8Length = 256;
    std::string text     = "# Title\n\nParagraph with\n\nbreaks\n\n## Section\n\nMore text";
    auto        result   = VectorStore::splitTextToChunks(text, config);
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks StructuralThenCharThenFixed, got " << result.size()
                  << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_default_config() {
    VectorStore::SplitConfig config;
    auto result = VectorStore::splitTextToChunks("Hello World\n\nThis is a test", config);
    if (result.size() >= 1) {
        g_rag_passed++;
        TEST_PASS << "splitTextToChunks default config" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks default config, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_chinese_character() {
    ...
    if (result.size() == 3) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks Chinese character, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_markdown_code() {
    ...
    if (hasCodeBlock && result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks markdown code, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_short_text_all_modes() {
    ...
    for (auto mode :
        ...
        if (result.size() != 1 || result[0] != "Hello") {
            g_rag_failed++;
            TEST_FAIL << "splitTextToChunks short text mode=" << static_cast<int>(mode) << ", got "
...
asio::awaitable<void> test_chunks_custom_delimiters() {
    ...
    if (result.size() == 3 && result[0] == "apple" && result[1] == "banana"
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks custom delimiters, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_chinese_structural() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks Chinese structural, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_numbered_list() {
    ...
    if (result.size() >= 1) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks numbered list, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_mixed_content() {
    ...
    if (result.size() >= 3) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks mixed content, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
    ...
    if (result.size() >= 2) {
        for (size_t i = 1; i < result.size(); ++i) {
            if (result[i - 1].size() >= 2
                && result[i - 1].substr(result[i - 1].size() - 2) != result[i].substr(0, 2)) {
                hasOverlap = false;
                break;
            }
        }
...
asio::awaitable<void> test_fixed_length_overlap_zero() {
    ...
    if (resultNoOverlap.size() == resultOverlap.size() && resultNoOverlap == resultOverlap) {
        g_rag_passed++;
        TEST_PASS << "splitByFixedLength overlap zero (disabled)" << std::endl;
    } else {
        g_rag_failed++;
        TEST_FAIL << "splitByFixedLength overlap zero, sizes: " << resultNoOverlap.size() << " vs "
                  << resultOverlap.size() << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_overlap_single_chunk() {
    ...
    if (result.size() == 1 && result[0] == "Hello") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByFixedLength overlap single chunk, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_fixed_length_overlap_utf8() {
    ...
    if (result.size() >= 3) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitByFixedLength overlap utf8, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_apply_chunk_overlap_basic() {
    ...
    if (result.size() == 3 && result[0] == "AAAAABBBBB" && result[1] == "BBCCCCCDDDDD"
...
asio::awaitable<void> test_apply_chunk_overlap_single() {
    ...
    if (result.size() == 1 && result[0] == "HelloWorld") {
        ...
        g_rag_failed++;
        TEST_FAIL << "applyChunkOverlap single chunk, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_apply_chunk_overlap_empty() {
    ...
    if (result.empty()) {
        ...
        g_rag_failed++;
        TEST_FAIL << "applyChunkOverlap empty, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_apply_chunk_overlap_short_prev() {
    ...
    if (result.size() == 3 && result[0] == "AB" && result[1] == "CCCCCDDDDD"
        ...
        g_rag_failed++;
        TEST_FAIL << "applyChunkOverlap short prev, got " << result.size() << " chunks:";
        for (auto& s : result) {
            std::cout << " [" << s << "]";
        }
        std::cout << std::endl;
...
asio::awaitable<void> test_apply_chunk_overlap_utf8() {
    ...
    if (result.size() == 2 && result[0] == "你好世界甲乙丙丁"
        ...
        g_rag_failed++;
        TEST_FAIL << "applyChunkOverlap utf8, got " << result.size() << " chunks" << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_overlap_fixed_length() {
    ...
    auto        result          = VectorStore::splitTextToChunks(text, config);
    auto        resultNoOverlap = VectorStore::splitByFixedLength(text, config.maxUtf8Length);
...
asio::awaitable<void> test_chunks_overlap_character() {
    ...
    if (result.size() >= 4) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks overlap Character, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_overlap_structural() {
    ...
    if (result.size() >= 2) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks overlap Structural, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_overlap_zero_disabled() {
    ...
    if (result.size() == resultDefault.size() && result == resultDefault) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks overlap zero disabled, sizes: " << result.size() << " vs "
                  << resultDefault.size() << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_overlap_default_config() {
    ...
    if (result.size() >= 1) {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks overlap default config, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_overlap_short_text() {
    ...
    if (result.size() == 1 && result[0] == "Hello") {
        ...
        g_rag_failed++;
        TEST_FAIL << "splitTextToChunks overlap short text, got " << result.size() << " chunks"
                  << std::endl;
    }
    co_return;
}
...
asio::awaitable<void> test_chunks_overlap_all_modes() {
    ...
    for (auto mode :
        ...
        if (result.empty()) {
            g_rag_failed++;
            TEST_FAIL << "splitTextToChunks overlap all modes, mode=" << static_cast<int>(mode)
                      << " returned empty" << std::endl;
            co_return;
        }
    }
    g_rag_passed++;
...
asio::awaitable<void> test_cosine_partial_overlap() {
    ...
    if (std::abs(sim - expected) < 0.0001) {
        ...
        g_rag_failed++;
        TEST_FAIL << "cosineSimilarity partial overlap, sim=" << sim << ", expected=" << expected
                  << std::endl;
    }
    co_return;
}
