...
_Static_assert(offsetof(AgentxxPluginSchedulerIface, version) == 0, "SchedulerIface.version 首字段");
_Static_assert(
...
enum {
    ...
    AGENTXX_ABI_VALUE_VALUE_COUNT               = 18
};
...
uint64_t agentxx_test_abi_value(int32_t id) {
    switch (id) {
        case AGENTXX_ABI_VALUE_STRING_VIEW_SIZE: return (uint64_t)sizeof(AgentxxPluginStringView);
        case AGENTXX_ABI_VALUE_STRING_VIEW_SIZE_OFFSET:
            return (uint64_t)offsetof(AgentxxPluginStringView, size);
        case AGENTXX_ABI_VALUE_PLUGIN_HOST_SIZE: return (uint64_t)sizeof(AgentxxPluginHost);
        case AGENTXX_ABI_VALUE_PLUGIN_HOST_OPAQUE_OFFSET:
            return (uint64_t)offsetof(AgentxxPluginHost, opaque);
        case AGENTXX_ABI_VALUE_HOST_VTABLE_SIZE: return (uint64_t)sizeof(AgentxxHostVtable);
        case AGENTXX_ABI_VALUE_TOOL_SPEC_SIZE: return (uint64_t)sizeof(AgentxxPluginToolSpec);
        case AGENTXX_ABI_VALUE_TOOL_SPEC_START_OFFSET:
            return (uint64_t)offsetof(AgentxxPluginToolSpec, execute_start);
        case AGENTXX_ABI_VALUE_HOOK_SPEC_SIZE: return (uint64_t)sizeof(AgentxxPluginHookSpec);
        case AGENTXX_ABI_VALUE_NOTIFY_SIZE: return (uint64_t)sizeof(AgentxxPluginOperatorNotify);
        ...
        case AGENTXX_ABI_VALUE_AGENT_API_VERSION: return (uint64_t)AGENTXX_PLUGIN_API_VERSION;
