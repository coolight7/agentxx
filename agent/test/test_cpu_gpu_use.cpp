...
asio::awaitable<TestResult> run_cpu_gpu_use_tests() {
    ...
    if (usage.cpuUsagePercent >= 0.0 && usage.cpuUsagePercent <= 100.0) {
        ...
        g_cpu_failed++;
        TEST_FAIL << "cpuUsagePercent 超出范围: " << usage.cpuUsagePercent << std::endl;
        co_return TestResult{g_cpu_passed, g_cpu_failed};
    }

    TEST_INFO << "内存总量: " << usage.memory.totalPhysicalMB << " MB" << std::endl;
    TEST_INFO << "已用内存: " << usage.memory.usedPhysicalMB << " MB" << std::endl;
    ...
    if (usage.memory.usagePercent >= 0.0 && usage.memory.usagePercent <= 100.0) {
        ...
        g_cpu_failed++;
        TEST_FAIL << "memory.usagePercent 超出范围: " << usage.memory.usagePercent << std::endl;
        co_return TestResult{g_cpu_passed, g_cpu_failed};
    }
    ...
    for (size_t i = 0; i < usage.gpus.size(); ++i) {
        ...
        TEST_INFO << "  专用显存: " << gpu.dedicatedVramUsedMB << " / " << gpu.dedicatedVramMB
                  << " MB" << std::endl;
        TEST_INFO << "  共享显存: " << gpu.sharedVramUsedMB << " / " << gpu.sharedVramMB << " MB"
                  << std::endl;
        ...
        if (gpu.dedicatedVramUsedMB <= gpu.dedicatedVramMB) {
            ...
            g_cpu_failed++;
            TEST_FAIL << "GPU[" << i << "] 专用显存已用 > 总量: " << gpu.dedicatedVramUsedMB
        ...
        if (gpu.usagePercent >= 0.0 && gpu.usagePercent <= 100.0) {
            ...
            g_cpu_failed++;
            TEST_FAIL << "GPU[" << i << "] usagePercent 超出范围: " << gpu.usagePercent
                      << std::endl;
            co_return TestResult{g_cpu_passed, g_cpu_failed};
        }
    }
