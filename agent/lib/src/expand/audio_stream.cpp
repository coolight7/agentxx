...
public:
    ...
    Impl() :
        running_(false),
        source_(AudioDataSource::SystemOutput),
        targetProcessId_(0) {}
    ...
    bool start(AudioDataSource source, uint32_t targetProcessId) {
        ...
        if (FAILED(hr) && hr != RPC_E_CHANGED_MODE) {
            XX_LOGE("AudioStream: CoInitializeEx failed, hr=0x{:08X}", static_cast<unsigned>(hr));
            return false;
        }
        ...
        hr = activateDevice();
        if (FAILED(hr)) {
            XX_LOGE("AudioStream: activateDevice failed, hr=0x{:08X}", static_cast<unsigned>(hr));
            cleanupCom();
            return false;
        }
        ...
        hr = initializeClient();
        if (FAILED(hr)) {
            XX_LOGE("AudioStream: initializeClient failed, hr=0x{:08X}", static_cast<unsigned>(hr));
            cleanupResources();
            return false;
        }
...
private:
    ...
    HRESULT initializeClient() {
        ...
        if (FAILED(hr)) {
            XX_LOGE("AudioStream: GetMixFormat failed, hr=0x{:08X}", static_cast<unsigned>(hr));
            return hr;
        }
        ...
        if (source_ == AudioDataSource::SystemOutput || source_ == AudioDataSource::ProgramOutput) {
        ...
        if (FAILED(hr)) {
            XX_LOGE("AudioStream: SetEventHandle failed, hr=0x{:08X}", static_cast<unsigned>(hr));
            CoTaskMemFree(mixFormat);
            return hr;
        }
    ...
    HRESULT readCaptureBuffer() {
        ...
        while (packetLength != 0) {
            ...
            if (numFrames > 0 && data != nullptr && !(flags & AUDCLNT_BUFFERFLAGS_SILENT)) {
    ...
    bool isTargetProcessActive() {
        ...
        HRESULT hr = sessionManager_->GetSessionEnumerator(enumerator.GetAddressOf());
        if (FAILED(hr)) {
            return true;
        }
...
void AudioStream::removeAllListeners() {
