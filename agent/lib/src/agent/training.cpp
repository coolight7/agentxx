...
size_t PromptVariant::promptHash() const { return prompt.promptHash(); }
...
void EvolutionTrainingAgent::applyVariantToTrainAgent(
...
neograph::json
EvolutionTrainingAgent::promptVariantToJson(const PromptVariant &v) const {
  neograph::json j;
  j["id"] = v.id;
...
PromptVariant
EvolutionTrainingAgent::promptVariantFromJson(const neograph::json &j) const {
  PromptVariant v;
...
void EvolutionTrainingAgent::rotateSaveFile(const std::string &path,
                                            int keepCount) {
  if (keepCount <= 0) {
    return;
  }
...
void EvolutionTrainingAgent::savePopulationToFile(const std::string &filePath,
                                                  int backupCount) {
  try {
    if (backupCount > 0) {
...
bool EvolutionTrainingAgent::loadPopulationFromFile(
...
EvolutionTrainingAgent::defaultScoringWithSubAgent(
    std::string_view agentOutput, const TrainingTestCase &testCase,
    int iteration, const EvolutionTrainingConfig &cfg) {
...
std::string EvolutionTrainingAgent::buildPromptContextMessage(
    const PromptVariant &variant) const {
...
EvolutionTrainingAgent::optimizeVariantWithLLM(
...
std::string EvolutionTrainingAgent::mutateString(const std::string &input,
                                                 double mutationRate) {
...
PromptVariant
EvolutionTrainingAgent::createChildVariantCharMut(const PromptVariant &parent,
                                                  double mutationRate) {
  PromptVariant child;
  child.id = generateId();
...
EvolutionTrainingAgent::createChildVariantLLMMut(
    const PromptVariant &parent, const EvolutionTrainingConfig &cfg) {
  PromptVariant child;
  child.id = generateId();
  child.prompt = parent.prompt;
  child.generation = generationCounter;
...
void EvolutionTrainingAgent::seedInitialPopulation(
...
asio::awaitable<void>
EvolutionTrainingAgent::runEvolutionLoop(const EvolutionTrainingConfig &cfg) {
...
const std::vector<PromptVariant> &
EvolutionTrainingAgent::getPopulation() const {
  return population;
}
...
void EvolutionTrainingAgent::applyBestPromptToConfig(
