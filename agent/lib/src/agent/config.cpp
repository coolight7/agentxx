...
std::expected<void, std::string> AgentConfig::validate() const {
    if (!model.isValid() && availableModels.empty()) {
        return std::unexpected{"AgentConfig: no valid model (model.baseUrl/apiKey empty and availableModels empty)"};
    }
    if (!availableModels.empty() && !currentModelName.empty() && !availableModels.contains(currentModelName)) {
        return std::unexpected{fmt::format("AgentConfig: currentModelName '{}' not in availableModels", currentModelName)};
    }
