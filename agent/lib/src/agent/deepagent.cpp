...
asio::awaitable<void> DeepAgent::init() {
...

  {
    ...
    /// agentContext->bus
    agentContext->bus =
        std::make_shared<agentxx::middleware::EventBus>(ioCtx->get_executor());
  }
  ...
  auto subagentManagerTool =
      std::make_unique<agentxx::tools::SubAgentManagerTool>("subagent_manager",
                                                            agentContext);
  ...
  }
  {
    ...
    tools.push_back(
        std::make_unique<agentxx::tools::GetCurrentDateTimeTool>(agentContext));
#if XX_IS_WIN_D || XX_IS_LINUX_D
    tools.push_back(
        std::make_unique<agentxx::tools::GetSystemCoreInfoTool>(agentContext));
#endif
    ...
    if (false == config->ragDocsPaths.empty()) {
      auto client = std::make_shared<agentxx::tools::EmbeddingClient>(
          config->model.baseUrl, config->model.apiKey, config->model.modelName);
      auto docsStore =
          std::make_shared<agentxx::tools::RAGSearchTool::VectorStore>(client);
...
┗━━━━━━ RAG Embedding ━━━━━━┛
    ...
    tools.push_back(
        std::make_unique<agentxx::tools::UIControlKeyboardMouseTool>(
            agentContext));
    tools.push_back(std::make_unique<agentxx::tools::ExecuteWindowsCommandTool>(
        agentContext));
#elif XX_IS_LINUX_D
...

    {
      ...
      tools.push_back(
          std::make_unique<agentxx::tools::CrossAgentQueryTool>(agentContext));
    }

    {
      // subagent
      {
        ...
        nodeContext.provider =
            ModelProviderRegistry::createProvider(subagentModelCfg);
  ...
  engine = std::move(neograph::graph::GraphEngine::compile(graphDefinition,
                                                           nodeContext, store));
  {
...
void DeepAgent::selectModel(const std::string &threadId,
...
asio::awaitable<DeepAgent::ConversationTurnResult>
DeepAgent::runConversationTurnAsync(
    const std::string &threadId, const std::string &userInput, bool isFirstMsg,
    neograph::json messages, std::shared_ptr<AgentIOBase> io,
    std::function<void(const neograph::graph::GraphEvent &)> eventCallback,
    InterruptCallback interruptCallback, const std::string &modelName) {
      ...
      for (const auto &interruptArg : interruptArgs) {
        ...
        std::optional<neograph::json> interruptResult;
        {
          ...
          if (interruptArg.name == "subagent") {
            ...
            auto batchResp =
                co_await agentContext->bus->request<events::ReqSubagentBatch,
...
DeepAgent::~DeepAgent() { engine = nullptr; }

neograph::graph::GraphEngine *DeepAgent::getEngine() { return engine.get(); }

const neograph::graph::GraphEngine *DeepAgent::getEngine() const {
  return engine.get();
}

std::shared_ptr<AgentContext> DeepAgent::getContext() { return agentContext; }
...
asio::awaitable<std::string> DeepAgent::runNonStreamAsync(
  ...
  auto wrappedCallback = [&oss,
                          callback](const neograph::graph::GraphEvent &event) {
    switch (event.type) {
...
asio::awaitable<std::string> DeepAgent::runSingleInputAsync(
    ...
    const std::string &systemPrompt, const std::string &modelName) {
