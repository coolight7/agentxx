...
//   load_for_resume_async → get_writes(thread_id, latest.id));
...
void SingleCheckpointStore::save(const Checkpoint& cp) {
    // 先持久化为该 thread 的最新 checkpoint
    saveImpl(cp);
    // 再淘汰该 thread 除最新 checkpoint 外的全部历史数据
...
void InMemorySingleCheckpointStore::saveImpl(const Checkpoint& cp) {
    ...
    // 每个 thread 仅保留最新 checkpoint, 直接覆盖
...
std::optional<Checkpoint> InMemorySingleCheckpointStore::load_by_id(const std::string& id) {
    std::lock_guard lock(mutex_);
    // 仅可能命中某个 thread 的最新 checkpoint; 历史 id 已被淘汰
...
void InMemorySingleCheckpointStore::delete_thread(const std::string& thread_id) {
    std::lock_guard lock(mutex_);
    latest_.erase(thread_id);
    // 同步清理该 thread 的全部 pending writes
    for (auto it = pending_.begin(); it != pending_.end();) {
        if (it->first.first == thread_id) {
            it = pending_.erase(it);
        } else {
            ++it;
        }
