...
ChannelTransport::makePair(asio::any_io_executor clientEx, asio::any_io_executor serverEx, size_t cap) {
    // client->server 通道由 server 端接收 (serverEx); server->client 通道由 client 端接收 (clientEx)
...
ChannelTransport::send(std::string_view jsonText) {
