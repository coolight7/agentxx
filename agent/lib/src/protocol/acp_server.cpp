...
AcpProtocolHandler::AcpProtocolHandler(
    std::shared_ptr<agentxx::agent::DeepAgent> agent, json agentInfo,
    Config config)
    : config_(std::move(config)), deepAgent_(std::move(agent)),
      agentInfo_(std::move(agentInfo)) {}

AcpProtocolHandler::~AcpProtocolHandler() { stop(); }

bool AcpProtocolHandler::initialized() const {
  return initialized_.load(std::memory_order_acquire);
}

const json &AcpProtocolHandler::agentInfo() const { return agentInfo_; }
...
json AcpProtocolHandler::callClient(const std::string &method, json params,
...
json AcpProtocolHandler::jsonRpcError(const json &id, int code,
...
void AcpProtocolHandler::handleSessionPrompt(const json &env,
                                             const json &params,
                                             const json &id) {
...
void AcpProtocolHandler::workerRunPrompt(
    const std::string &sessionId, const json &promptBlocks, const json &id,
    std::shared_ptr<std::atomic<bool>> cancelFlag) {
  try {
...
void AcpProtocolHandler::emitNotification(const std::string &method,
                                          const json &params) {
  json env;
  env["jsonrpc"] = "2.0";
  env["method"] = method;
...
void AcpProtocolHandler::emitAgentMessageChunk(const std::string &sessionId,
...
HttpAcpServer::HttpAcpServer(std::shared_ptr<agentxx::agent::DeepAgent> agent,
...
HttpAcpServer::~HttpAcpServer() { stop(); }

void HttpAcpServer::start() { httpServer_->start(); }
...
uint16_t HttpAcpServer::port() const { return httpServer_->port(); }
bool HttpAcpServer::isStopped() const { return httpServer_->isStopped(); }

AcpProtocolHandler &HttpAcpServer::handler() { return handler_; }
...
asio::awaitable<void>
HttpAcpServer::handleAcpRequest(util::HttpServer::Request &req,
...
asio::awaitable<void>
HttpAcpServer::handleSseRequest(util::HttpServer::Request &req,
                                util::HttpServer::Response &resp) {
...
void HttpAcpServer::writeJsonResponse(util::HttpServer::Response &resp,
...
neograph::json HttpAcpServer::jsonRpcError(const neograph::json &id, int code,
...
StdioAcpServer::StdioAcpServer(
    std::shared_ptr<agentxx::agent::DeepAgent> agent, neograph::json agentInfo)
    : deepAgent_(std::move(agent)),
      handler_(
          deepAgent_, std::move(agentInfo),
          {.serverName = "agentxx-acp-stdio", .serverVersion = "0.1.0"}) {}

StdioAcpServer::~StdioAcpServer() { stop(); }

void StdioAcpServer::run() { run(std::cin, std::cout); }
...
void StdioAcpServer::stop() { handler_.stop(); }

bool StdioAcpServer::isRunning() const {
  return running_.load(std::memory_order_acquire);
}

AcpProtocolHandler &StdioAcpServer::handler() { return handler_; }
