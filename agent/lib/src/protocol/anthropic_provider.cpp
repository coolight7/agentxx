...
std::string AnthropicProvider::get_name() const { return "anthropic"; }
...
AnthropicProvider::invoke_format_data(const neograph::CompletionParams &params,
                                      neograph::FormatDataStreamCallback
                                          on_chunk) {
  if (on_chunk) {
...
AnthropicProvider::convertMessages(
    const std::vector<neograph::ChatMessage> &messages, bool sendThinking) {
  std::string system;
...
neograph::json AnthropicProvider::convertTools(
    const std::vector<neograph::ChatTool> &tools) {
  neograph::json arr = neograph::json::array();
...
AnthropicProvider::parseResponse(const neograph::json &resp) {
...
neograph::json
AnthropicProvider::buildBody(const neograph::CompletionParams &params) const {
  neograph::json body;
  body["model"] =
...
AnthropicProvider::ParsedEndpoint
AnthropicProvider::parseEndpoint(const std::string &base_url) {
  ParsedEndpoint ep;
  ep.scheme = "https";
  ep.port = 443;
...
asio::awaitable<neograph::ChatCompletion> AnthropicProvider::doStream(
    const neograph::CompletionParams &params, const neograph::json &body,
