...
McpServer::McpServer(Config config) : config_(std::move(config)) {
...
void McpServer::start() { httpServer_->start(); }
...
uint16_t McpServer::port() const { return httpServer_->port(); }
size_t McpServer::activeConnections() const {
  return httpServer_->activeConnections();
}
bool McpServer::isStopped() const { return httpServer_->isStopped(); }
...
void McpServer::setCapabilities(Capabilities caps) { capabilities_ = caps; }
...
asio::awaitable<void> McpServer::handleMcpRequest(
    util::HttpServer::Request &req, util::HttpServer::Response &resp) {
...
void McpServer::broadcastSSE(const std::string &event,
