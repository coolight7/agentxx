...
OpenAIProvider::create(const agentxx::agent::ModelConfig &config) {
...
std::string OpenAIProvider::get_name() const { return "openai"; }
...
neograph::json
OpenAIProvider::buildBody(const neograph::CompletionParams &params) const {
  neograph::json body;
...
OpenAIProvider::ParsedEndpoint
OpenAIProvider::parseEndpoint(const std::string &base_url) {
  ParsedEndpoint ep;
  ep.scheme = "https";
  ep.port = 443;
...
OpenAIProvider::completeAsync(const neograph::CompletionParams &params) {
  ...
  auto resp = co_await HttpClient::postAsync(
      config_.baseUrl + "/chat/completions", bodyStr, "application/json",
      headers,
...
void OpenAIProvider::extractThinkTags(std::string &content,
                                      std::string &thinking) {
  std::string cleaned;
  size_t pos = 0;
