...
McpClient::McpClient(Config config) : config_(std::move(config)) {}

McpClient::~McpClient() { closeInternal(); }

asio::awaitable<std::expected<McpClient::InitializeResult, std::string>>
...
bool McpClient::isInitialized() const { return initialized_.load(); }
bool McpClient::isClosed() const { return closed_.load(); }
...
McpClient::getErrorFromResponse(const json &response) {
...
std::vector<McpClient::SseEvent>
McpClient::parseSseEvents(const std::string &body) {
...
std::string McpClient::getQueryParam(std::string_view query,
                                     std::string_view key) {
...
asio::awaitable<std::expected<json, std::string>>
McpClient::sendHttpRequest(int64_t id, const std::string &method,
                           const json &params) {
  if (!sseDiscovered_.load())
...
asio::awaitable<std::expected<json, std::string>>
McpClient::sendStdioRequest(int64_t id, const std::string &method,
                            const json &params) {
  auto req = makeRequest(id, method, params);
...
McpClientTool::execute_async(const neograph::json &arguments) {
