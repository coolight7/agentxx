...
EmbeddingClient::embed_batch(const std::vector<std::string> &texts) const {
  ...
  auto resp = co_await agentxx::util::HttpClient::postAsync(
      ...
      agentxx::util::HttpClient::RequestConfig{.readTimeout =
...
double RAGSearchTool::cosineSimilarity(const std::vector<double> &a,
...
RAGSearchTool::VectorStore::VectorStore(
    ...
    : splitConfig(in_splitCfg), embedder(std::move(in_embedder)) {}

std::vector<std::string> RAGSearchTool::VectorStore::splitByFixedLength(
    std::string_view text, size_t blockSize, double overlapPercent) {
...
RAGSearchTool::VectorStore::splitByDelimiter(std::string_view text,
...
std::vector<std::string>
RAGSearchTool::VectorStore::splitByStructure(std::string_view text) {
...
std::vector<std::string> RAGSearchTool::VectorStore::splitByDelimiters(
    std::string_view text, size_t maxUtf8Length,
...
std::vector<std::string> RAGSearchTool::VectorStore::applyChunkOverlap(
    const std::vector<std::string> &chunks, size_t maxUtf8Length,
...
std::vector<std::string>
RAGSearchTool::VectorStore::splitTextToChunks(std::string_view text,
...
RAGSearchTool::VectorStore::scanDocument(
...
asio::awaitable<bool>
RAGSearchTool::VectorStore::addDocuments(std::vector<Document> &&appendDocs) {
  if (appendDocs.empty()) {
    co_return true;
  }
...
RAGSearchTool::RAGSearchTool(
    ...
    : XXToolBase("rag_search", in_agentContext, false, true), store(in_store) {}
...
RAGSearchTool::execute_async(const neograph::json &arguments) {
