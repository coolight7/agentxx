...
WebSearchTool::execute_async(const neograph::json &arguments) {
...
WebFetchUrlTool::WebFetchUrlTool(
...
asio::awaitable<std::string>
WebFetchUrlTool::execute_async(const neograph::json &arguments) {
  ...
  auto resp = co_await agentxx::util::HttpClient::getAsync(
      url, {},
      agentxx::util::HttpClient::RequestConfig{
          .readTimeout = std::chrono::seconds(timeout)});
  if (resp.has_value()) {
...
asio::awaitable<std::string>
ModelWebSearchTool::execute_async(const neograph::json &arguments) {
  std::string query = arguments.value("query", std::string{});
  if (query.empty()) {
    co_return R"({"error":"Arg `query` is empty"})";
  }
