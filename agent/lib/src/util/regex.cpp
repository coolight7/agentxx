...
const unsigned int agentxx::util::XXRegex::defHSFlags_onlyContains = HS_FLAG_UTF8 | HS_FLAG_UCP;
...
public:
    ...
    inline static int xxregexMatchCallback(
        ...
        if (!results->empty()) {
            ...
            std::vector<typename std::vector<agentxx::util::XXRegexMatchResult>::iterator> to_erase;
...
public:
    ...
    bool match(std::string_view input, std::vector<agentxx::util::XXRegexMatchResult>& results)
        ...
        // 合并重叠区间（不合并相邻区间）
        agentxx::util::XXRegexMatchResult current{raw_matches[0].first, raw_matches[0].second};
