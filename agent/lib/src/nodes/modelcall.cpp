...
std::string
ModelCallWrapNode::resolveCurrentModelName(const std::string &threadId) const {
  if (useDynamicModel_) {
    auto ctxPtr = agentContext.lock();
    if (ctxPtr && ctxPtr->modelRegistry) {
      std::string selected;
...
ModelCallWrapNode::build_params(const neograph::graph::GraphState &state,
...
void ModelCallWrapNode::onHandleStartError(
    bool errorRethrow, bool isCurrentError, std::string_view exceptionStr,
    agentxx::middleware::BaseMiddlewareHandleInterface &item,
    neograph::graph::NodeInput &in,
...
asio::awaitable<void> ModelCallWrapNode::baseRun(
    std::vector<
        std::shared_ptr<agentxx::middleware::BaseMiddlewareHandleInterface>>
        &handles,
