...
size_t
SummarizationMiddlewareHandle::countTokensForUtf8Str(std::string_view in_str) {
...
std::string SummarizationMiddlewareHandle::messagesToText(
    const std::vector<neograph::ChatMessage> &msgs, bool includeSystem) {
...
void SummarizationMiddlewareHandle::offloadLongContentToTempStore(
    neograph::ChatMessage &msg, const std::shared_ptr<MiddlewareContext> &ctx,
...
asio::awaitable<void> SummarizationMiddlewareHandle::onModelcallRunFunc(
  ...
  if (tokenUsage >= modelSupportMaxToken * 0.65) {
    doSummarizeToolcall(messages);
    {
      for (auto &msg : messages) {
        offloadLongContentToTempStore(msg, agentCtxPtr->middlewareHandleContext,
                                      thread_id);
      }
    }
