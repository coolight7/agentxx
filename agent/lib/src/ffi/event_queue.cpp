    ...
    }
    {
        ...
        if (q->items.size() >= AgentxxFFIEventQueue::kMaxQueued) {
            ...
            q->items.emplace_back(
                static_cast<int32_t>(AGENTXX_FFI_EVT_ERROR),
                std::string(
...
int32_t AGENTXX_FFI_CALL agentxx_ffi_event_queue_pop(
    ...
    std::pair<int32_t, std::string> item;
    {
        ...
        if (!q->cv.wait_for(lock, std::chrono::milliseconds{timeout_ms}, [&] {
            ...
            return AGENTXX_FFI_ERR_TIMEOUT; // 等待超时且无事件
        }
        if (q->closed && q->items.empty()) {
            return AGENTXX_FFI_ERR_STATE; // 队列已销毁
        }
    ...
    *type_out = item.first;
    ...
    try {
        ...
        out            = static_cast<char*>(agentxx_ffi_malloc(n + 1));
        if (out == nullptr) {
