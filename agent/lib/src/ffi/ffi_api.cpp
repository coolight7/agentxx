...
// 用后必须 agentxx_free 释放)。符号导出白名单见 lib/ffi_symbols.map。
...
int ffiFail(int code, const std::string& detail, char** log) {
    if (log != nullptr) {
        *log = agentxx_strdup_n(detail.data(), detail.size());
    }
    return code;
}
...
// 头文件仅有 typedef struct AgentxxAgent AgentxxAgent 前置声明, 此处补充完整定义
...
char* agentxx_strdup_n(const char* s, size_t size) {
    if (s == nullptr) {
        return nullptr;
    }
    auto* p = static_cast<char*>(agentxx_malloc(size + 1));
    if (p == nullptr) {
        return nullptr;
    }
    if (size > 0) {
...
const char* agentxx_ffi_strerror(int code) {
    switch (code) {
        case AGENTXX_OK:
            return "success";
        case AGENTXX_ERR_INVALID:
            return "invalid argument";
        case AGENTXX_ERR_STATE:
            return "invalid state";
        case AGENTXX_ERR_JSON:
            return "JSON parse failed";
        case AGENTXX_ERR_CONFIG:
            return "invalid configuration";
        case AGENTXX_ERR_INIT:
            return "agent init failed";
        case AGENTXX_ERR_INTERRUPT:
            return "interrupt id invalid/expired";
        case AGENTXX_ERR_TIMEOUT:
            return "sync query timeout";
        case AGENTXX_ERR_OOM:
            return "out of memory";
        case AGENTXX_ERR_INTERNAL:
...
AgentxxAgent* agentxx_create(
    ...
    try {
        ...
        if (!rt) {
            ffiFail(AGENTXX_ERR_CONFIG, err, log);
            return nullptr;
        }
        auto* handle = new (std::nothrow) AgentxxAgent();
        if (handle == nullptr) {
            err = "out of memory";
            ffiFail(AGENTXX_ERR_OOM, err, log);
            return nullptr;
        }
        ...
        ffiFail(AGENTXX_ERR_INTERNAL, cxxErrText(), log);
        return nullptr;
    }
}

int agentxx_start(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
...
int agentxx_stop(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
        return ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
    }
    std::string err;
...
int agentxx_destroy(AgentxxAgent* a, char** log) {
    if (a == nullptr) {
        return ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
    }
    int         rc = AGENTXX_OK;
...
int agentxx_send_input(AgentxxAgent* a, const char* text, char** log) {
    if (a == nullptr || !a->impl) {
        return ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
    }
    if (text == nullptr) {
        return ffiFail(AGENTXX_ERR_INVALID, "null text", log);
    }
    std::string err;
    try {
...
int agentxx_cancel(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
        return ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
    }
    std::string err;
...
int agentxx_select_model(AgentxxAgent* a, const char* model_name, char** log) {
    if (a == nullptr || !a->impl) {
        return ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
    }
    if (model_name == nullptr) {
        return ffiFail(AGENTXX_ERR_INVALID, "null model_name", log);
    }
    std::string err;
    try {
...
int agentxx_set_permission(AgentxxAgent* a, const char* path, int allow, int op, char** log) {
    ...
    if (path == nullptr) {
        return ffiFail(AGENTXX_ERR_INVALID, "null path", log);
    }
    std::string err;
    try {
...
char* agentxx_get_model_info(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
        ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
        return nullptr;
    }
    std::string err, out;
    try {
    ...
    if (out.empty()) {
        ffiFail(AGENTXX_ERR_TIMEOUT, err, log);
        return nullptr;
    }
    ffiOk(log);
    return agentxx_strdup_n(out.data(), out.size());
}

char* agentxx_get_context_messages(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
        ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
        return nullptr;
    }
...
char* agentxx_list_sessions(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
        ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
        return nullptr;
    }
...
char* agentxx_drain_logs(AgentxxAgent* a, char** log) {
    if (a == nullptr || !a->impl) {
        ffiFail(AGENTXX_ERR_INVALID, "null handle", log);
        return nullptr;
    }
