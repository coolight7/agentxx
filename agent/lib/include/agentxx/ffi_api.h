...
AGENTXX_FFI_EXPORT int agentxx_set_permission(
    AgentxxAgent* a, const char* path, int allow, int op, char** log
);
...
AGENTXX_FFI_EXPORT int agentxx_interrupt_respond(
    AgentxxAgent* a, int64_t interrupt_id, const char* values_json, char** log
);
