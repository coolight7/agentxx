 ...
 *   - 所有事件经 AgentxxFFICallbacks::on_event 回调, payload 为 JSON 字符串
 ...
 *     agentxx_ffi_stop/agentxx_ffi_destroy (见下)
 *
 * 错误处理约定:
 *   - 同步错误: 返回值 = 错误码 (AGENTXX_FFI_OK=0 成功, 见 AGENTXX_ERR_*);
 *     可选的 char** log 参数非 NULL 时, 函数会 (按需) 填入 NUL 结尾 UTF-8
 *     详情字符串, 宿主用后必须 agentxx_ffi_free 释放; 成功/无日志时 *log 为 NULL
 ...
 *
 * 内存: 宿主分配的内存经 agentxx_ffi_free 释放 (跨 CRT 堆边界唯一通道,
...
AGENTXX_FFI_EXPORT void* agentxx_ffi_malloc(size_t size);   ///< 分配
AGENTXX_FFI_EXPORT void  agentxx_ffi_free(const void* ptr); ///< 释放
/// 分配并拷贝 NUL 结尾字符串 (字符串返回值的统一分配通道)
AGENTXX_FFI_EXPORT char* agentxx_ffi_strdup_n(const char* s, size_t size);
...
typedef enum AgentxxFFIEventType {
    ...
    AGENTXX_FFI_EVT_COMPONENTS, ///< 启动组件 (MCP/Skill/Memory/插件): wire append_component_info
                                ///< JSON
    AGENTXX_FFI_EVT_INTERRUPT_REQ,     ///< HIL 中断询问 (权限确认/输入收集): 见注释
...
typedef struct AgentxxFFICallbacks {
    ...
    void (*on_event)(AgentxxFFIEventType type, const char* payload_json, void* user_data);
 ...
 * @return 句柄; 失败返回 NULL
 */
AGENTXX_FFI_EXPORT AgentxxFFIAgent* agentxx_ffi_create(
...
AGENTXX_FFI_EXPORT int agentxx_ffi_start(AgentxxFFIAgent* a, char** log);

/// 同步停止并回收 (阻塞到 agent io 线程退出; 幂等; 不得在回调线程内调用)
AGENTXX_FFI_EXPORT int agentxx_ffi_stop(AgentxxFFIAgent* a, char** log);

/// 销毁句柄 (未 stop 时自动 stop; 之后句柄失效; 不得在回调线程内调用)
AGENTXX_FFI_EXPORT int agentxx_ffi_destroy(AgentxxFFIAgent* a, char** log);

/* ==================== 会话交互 (异步, 投递 io 线程执行) ==================== */

/// 发送用户输入 (EVT_READY 前发送会缓存, 就绪后按序消费)
AGENTXX_FFI_EXPORT int agentxx_ffi_send_input(AgentxxFFIAgent* a, const char* text, char** log);

/// 请求取消当前轮次
AGENTXX_FFI_EXPORT int agentxx_ffi_cancel(AgentxxFFIAgent* a, char** log);

/// 切换当前会话所用模型 (结果经 EVT_MODEL_INFO 通知)
AGENTXX_FFI_EXPORT int
    agentxx_ffi_select_model(AgentxxFFIAgent* a, const char* model_name, char** log);

/// 记住权限选择 (服务端注册路径规则, 后续同路径不再询问):
/// op: 0=读取 1=写入; allow: 1=允许 0=拒绝
AGENTXX_FFI_EXPORT int
    agentxx_ffi_set_permission(AgentxxFFIAgent* a, const char* path, int allow, int op, char** log);

/// 切换当前连接会话 (sessionId 为空 = 关闭持久化时非法):
/// 重新绑定会话并回推 Sync/ModelInfo/ContextStats (经对应事件通知)
AGENTXX_FFI_EXPORT int
    agentxx_ffi_switch_session(AgentxxFFIAgent* a, const char* sessionId, char** log);

/* ==================== 同步查询 (阻塞等待服务端响应, 最长 10s) ====================
 * 返回值: JSON 字符串 (agentxx_ffi_free 释放); 失败返回 NULL (log 含详情)
 * 注意: 同一句柄同一时刻仅允许一个在途 (服务端应答为逐条协议) */

/// 当前模型信息: {"currentModel","models":[...]}
AGENTXX_FFI_EXPORT char* agentxx_ffi_get_model_info(AgentxxFFIAgent* a, char** log);

/// 当前会话 LLM 上下文消息: {"messages":[chat message...]}
AGENTXX_FFI_EXPORT char* agentxx_ffi_get_context_messages(AgentxxFFIAgent* a, char** log);

/// 持久化会话列表: {"sessions":[{"sessionId","title","lastActiveMs"},...]}
AGENTXX_FFI_EXPORT char* agentxx_ffi_list_sessions(AgentxxFFIAgent* a, char** log);

/* ==================== HIL 中断应答 ==================== */

/// 提交 EVT_INTERRUPT_REQ 的应答 (values_json 见该事件注释; 任意线程可调用)
AGENTXX_FFI_EXPORT int agentxx_ffi_interrupt_respond(
    AgentxxFFIAgent* a,
...
AGENTXX_FFI_EXPORT char* agentxx_ffi_drain_logs(AgentxxFFIAgent* a, char** log);
 ...
 *   1. queue = agentxx_ffi_event_queue_create()
 *   2. cb.on_event = agentxx_ffi_event_queue_on_event; cb.user_data = queue
 *      (经 agentxx_ffi_create 注册, 桥接函数在 io 线程内同步拷贝 payload 入队)
 *   3. 宿主线程循环 agentxx_ffi_event_queue_pop 取事件 (json 用后 agentxx_ffi_free)
 *   4. 结束: agentxx_ffi_event_queue_free (自动丢弃积压事件)
...
AGENTXX_FFI_EXPORT AgentxxFFIEventQueue* agentxx_ffi_event_queue_create(void);

/// 销毁队列并释放积压事件 (唤醒全部等待者; 之后句柄失效; 勿与 pop 并发调用)
AGENTXX_FFI_EXPORT void agentxx_ffi_event_queue_free(AgentxxFFIEventQueue* q);

/// 内置 on_event 桥接实现 (user_data 必须为 agentxx_ffi_event_queue_create 返回值):
/// 与 AgentxxFFICallbacks.on_event 签名一致, 任意线程安全, 仅拷贝不阻塞 io
AGENTXX_FFI_EXPORT void agentxx_ffi_event_queue_on_event(
 ...
 */
AGENTXX_FFI_EXPORT int agentxx_ffi_event_queue_pop(
