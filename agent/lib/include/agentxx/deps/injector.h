...
private:
    ...
    std::shared_ptr<T> resolveTyped(std::string_view key) {
        ...
        try {
            ...
            throw std::runtime_error("Cannot cast dependency to requested type: " + std::string(key));
        }
    }
...
#define DEPS_RESOLVE(container, Type) \
    container.resolve<Type>()

#define DEPS_RESOLVE_NAMED(container, Type, name) \
    container.resolveNamed<Type>(name)
