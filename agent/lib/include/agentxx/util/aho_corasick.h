...
protected:
    ...
    inline static const auto caseShiftSheet = std::unordered_map<CharType, CharType>{
        {65, 97 },
        {66, 98 },
        {67, 99 },
        {68, 100},
...
public:
    ...
    const StringType& getPattern(size_t id) const {
        return patterns[id];
    }
    ...
    void build() {
        ...
        while (!q.empty()) {
            ...
            for (const auto [c, child] : nodes[curr].children) {
                ...
                nodes[child].fail = nodes[fail].children.contains(c) ? nodes[fail].children[c] : 0;
    ...
    [[nodiscard]] std::vector<AhoCorasick::AhoCorasickMatchInfo>
        ...
        for (size_t i = 0; i < text.length(); ++i) {
            ...
            // 跳转失败指针直到找到匹配或回到根节点
            while (current > 0 && false == nodes[current].children.contains(useChar)) {
