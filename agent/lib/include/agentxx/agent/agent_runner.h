...
public:
    ...
    struct Hooks {
        /// 中断发生时将 graphData 持久化到 engine checkpoint
        /// (程序重启恢复中断依赖; 根: true, 子代理一次性运行: false)
        ...
        /// engine state; 子代理: 空)
        std::function<asio::awaitable<void>(std::string_view threadId)> onBeforeResume;

        /// 每次 run/resume 返回后的回调 (根: 从中断保存的 tempMessages 恢复
        /// session llmMessages; 子代理: 空)
        std::function<void(neograph::graph::RunResult& result, std::string_view threadId)>
            onRunResult;
    };

    struct Outcome {
        /// 本轮是否发生过中断 (循环至少执行一次)
        /// - 根 agent (runConversationTurnAsync) 使用: 中断被处理并 resume
    ...
    /// @param ctx       agent 上下文 (主 agent 或子代理)
    /// @param engine    图引擎 (AgentContext 不持有, 由调用方传入)
    /// @param threadId  运行线程
    /// @param cfg       首跑配置 (resume_if_exists 等由调用方构造)
