...
struct DummyHandle {
    ...
    explicit DummyHandle(std::string_view n) :
        name(n) {}
};
...
inline void benchRouter() {
...

    {
        ...
        for (int i = 0; i < 1000; ++i) {
            auto handle = std::make_shared<DummyHandle>("handler_" + std::to_string(i));
            router.add("/api/v1/resource" + std::to_string(i), 0, handle);
        }

        auto r = runBench("XXRouter::get [1000 routes, cached lookup]", 100000, [&]() {
            std::string re_path;
...

    {
        ...
        auto r = runBench("XXRouter::getNocache [1000 routes, no cache]", 100000, [&]() {
            std::string re_path;
...

    {
        ...
        for (int i = 0; i < 100; ++i) {
            for (int j = 0; j < 10; ++j) {
                ...
                router
                    .add("/api/v" + std::to_string(i) + "/resource" + std::to_string(j), 0, handle);
            }
        }

        auto r = runBench("XXRouter::get [1000 routes, deep paths, cached]", 100000, [&]() {
            std::string re_path;
...

    {
        ...
        auto r = runBench("XXRouter::get [100 short routes, cached]", 500000, [&]() {
            std::string re_path;
...

    {
        ...
        auto r = runBench("XXRouter::get [10000 routes, cached lookup]", 50000, [&]() {
            std::string re_path;
...

    {
        ...
        auto r = runBench("XXRouter::getNocache [10000 routes, no cache]", 50000, [&]() {
            std::string re_path;
