...
inline void benchAhoCorasick() {
...

    {
        ...
        auto r = runBench("AhoCorasick::search [pre-built, 5 patterns, 500KB text]", 200, [&]() {
            auto result = ac.search(text);
            (void)result;
        });
        printResult(r);
...

    {
        ...
        auto r = runBench("AhoCorasick::removeAll [2 patterns, 250KB text]", 100, [&]() {
...

    {
        ...
        auto r = runBench("AhoCorasick::removeAll [HTML tag patterns, 250KB text]", 100, [&]() {
            auto result = ac.removeAll(text);
            (void)result;
        });
        printResult(r);
