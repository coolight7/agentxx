            ...
            [](agentxx::util::HttpServer::Request&  req,
               ...
               std::string_view) -> asio::awaitable<void> {
                ...
                if (stream) {
                    ...
                    usage["total_tokens"] = g_llm_sim_prompt_tokens + g_llm_sim_completion_tokens;
...
inline void reportBenchResult(const std::string& name, std::vector<double>& durations) {
    if (durations.empty()) {
        std::cout << "  [ERROR] No successful iterations for [" << name << "]" << std::endl;
        return;
    }
    ...
    double stddev_ns = std::sqrt(variance / static_cast<double>(durations.size()));
...
inline void benchCodeAgentRunConversationTurnAsync(const CodeAgentBenchConfig& cfg) {
    ...
    if (baseUrl.empty()) {
        ...
        apiKey    = "EMPTY";
        modelName = "bench-sim";
        std::cout << "  [INFO] Using local LLM simulator on 127.0.0.1:" << simGuard.port
                  << std::endl;
    }
        ...
        asio::co_spawn(
            ...
            [&]() -> asio::awaitable<void> {
                ...
                if (turnResult.hasError) {
                    ...
                    std::cout << "    [INFO] Iteration " << i << " completed" << std::endl;
                }
            },
            asio::detached
        );
...
inline void benchCodeAgentSimpleCompletion(const CodeAgentBenchConfig& cfg) {
    std::cout << "\n=== CodeAgent::runSingleInputAsync [simple completion] ===" << std::endl;
    ...
    for (size_t i = 0; i < cfg.iterations; ++i) {
        ...
        asio::co_spawn(
            ...
            [&]() -> asio::awaitable<void> {
                ...
                double ns  = std::chrono::duration<double, std::nano>(end - start).count();
                durations.push_back(ns);
                std::cout << "    [INFO] Iteration " << i << " completed, " << result.size()
                          << " chars" << std::endl;
            },
            asio::detached
        );
    ...
    for (size_t i = 0; i < cfg.iterations; ++i) {
        ...
        asio::co_spawn(
            ...
            [&]() -> asio::awaitable<void> {
                ...
                std::cout << "    [INFO] Iteration " << i << " completed (" << turnsPerIteration
                          << " turns)" << std::endl;
            },
            asio::detached
        );
        ...
        asio::co_spawn(
            ...
            [&]() -> asio::awaitable<void> {
                ...
                if (turnResult.hasError) {
                    ...
                    durations.push_back(ns);
                    std::cout << "    [INFO] Iteration " << i << " completed (" << historyMessages
                              << " history msgs + new turn)" << std::endl;
                }
            },
            asio::detached
        );
